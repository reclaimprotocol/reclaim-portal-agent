"""Compatibility shim — this module now lives in `agent.legacy.magic`.

Kept at its original path so the ~111 scripts and the production `genie/` app
keep importing `agent.magic` unchanged.

The alias below rebinds this module's entry in sys.modules to the real module
object, so `agent.magic is agent.legacy.magic`. Re-exporting names instead would
...
from agent.legacy import magic as _target
