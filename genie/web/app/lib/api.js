...
export async function searchPortals(q, limit = 20, country = "", state = "") {
  ...
  const r = await fetch(`${API_BASE}/search?${p.toString()}`);
...
export async function getStates(country = "India") {
  const r = await fetch(`${API_BASE}/states?country=${encodeURIComponent(country)}`);
...
export async function getUniversity(orgid) {
  const r = await fetch(`${API_BASE}/university?orgid=${encodeURIComponent(orgid)}`);
...
export async function browseUniversities({ offset = 0, limit = 40, country = "", state = "", q = "", onlyMissing = false } = {}) {
  ...
  const r = await fetch(`${API_BASE}/universities?${p.toString()}`);
...
export async function disputePortal({ orgid, url, reason = "", category = "", source = "", reasoning = "" }) {
  const r = await fetch(`${API_BASE}/dispute`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
export async function getDisputes({ offset = 0, limit = 50, q = "" } = {}) {
  const p = new URLSearchParams({ offset, limit });
  if (q) p.set("q", q);
  const r = await fetch(`${API_BASE}/training/disputes?${p.toString()}`);
  if (!r.ok) return { disputes: [], total: 0 };
  return r.json();
}

export async function updateDisputeComment(id, comment) {
  const r = await fetch(`${API_BASE}/training/disputes/${id}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
export async function deleteDispute(id) {
  const r = await fetch(`${API_BASE}/training/disputes/${id}`, { method: "DELETE" });
  if (!r.ok) throw new Error(`delete failed: ${r.status}`);
  return r.json();
}

export async function getTrainingStats() {
  const r = await fetch(`${API_BASE}/training/stats`);
  if (!r.ok) return {};
  return r.json();
}

export async function mineRules() {
  const r = await fetch(`${API_BASE}/training/mine`, { method: "POST" });
...
export async function getRules(status = "") {
  ...
  const r = await fetch(`${API_BASE}/training/rules${p}`);
  if (!r.ok) return { rules: [] };
  return r.json();
}

export async function createRule({ rule_type, pattern, action = "deny" }) {
  const r = await fetch(`${API_BASE}/training/rules/create`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
export async function getRuleTokens(url) {
  const r = await fetch(`${API_BASE}/training/tokens?url=${encodeURIComponent(url)}`);
  if (!r.ok) return { host: "", tokens: [] };
  return r.json();
}

export async function updateRule(id, { status, action } = {}) {
  const r = await fetch(`${API_BASE}/training/rules/${id}`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
export async function updatePortalCategory(id, category) {
  const r = await fetch(`${API_BASE}/portals/${id}/category`, {
    method: "POST", headers: { "Content-Type": "application/json" },
...
export async function getMetricsBatch(urls) {
  ...
  const r = await fetch(`${API_BASE}/metrics/batch`, {
    method: "POST",
...
export async function getMetrics(url) {
  const r = await fetch(`${API_BASE}/metrics?url=${encodeURIComponent(url)}`);
  if (!r.ok) return null;
  return r.json();
}

export async function startDiscover(url, includeAffiliated, { name = "", orgid = "", suppress = true } = {}) {
  const r = await fetch(`${API_BASE}/discover`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function lookupDb(url) {
  const r = await fetch(`${API_BASE}/lookup?url=${encodeURIComponent(url)}`);
